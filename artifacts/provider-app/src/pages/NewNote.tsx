...
export function NewNotePage({ patientId }: NewNotePageProps) {
  ...
  // Auto-generate the note as soon as the encounter ends. "Ended" =
  // recording stream is gone and at least one segment was captured,
  // while the pipeline is still idle (hasn't been kicked off and isn't
  // recovering from a failure the user might want to retry manually).
  // The ref guards against re-firing if the user resets while segments
  // are still present — they tapped reset deliberately.
